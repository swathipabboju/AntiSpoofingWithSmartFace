...
class FaceDetectorLandmarkPainter extends CustomPainter {
  FaceDetectorLandmarkPainter(
      this.absoluteImageSize, this.results, this.camPos);
    ...
    for (String label in results.keys) {
      for (final Face face in results[label]) {
        ...
        if (Platform.isAndroid && !camPos) {
          canvas.drawPoints(
              ...
              contour!.positionsList
                  .map((offset) => Offset(
                      size.width - (offset.dx * scaleX), offset.dy * scaleY))
                  .toList(),
              paint);
        ...
        if (Platform.isAndroid && !camPos) {
          ...
          textPainter.paint(
              canvas,
              new Offset((face.boundingBox.left.toDouble()) * scaleX,
                  (face.boundingBox.top.toDouble()) * scaleY));
        }
      }
    }
    ...
    for (String label in results.keys) {
      for (Face face in results[label]) {
        ...
        if (Platform.isIOS || camPos) {
          textPainter.paint(
              canvas,
              new Offset((10 + face.boundingBox.left.toDouble()) * scaleX!,
